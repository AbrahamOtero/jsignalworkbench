...
public class SignalManager {
   ...
   public void addListener(SignalSizeListener listener) {
     ...
     * @return True si existe la señal, False en caso de que no exista
     */
    ...
    public void renameSignal(String oldName,String newName) throws SignalNotFoundException{
        ...
        if(exists(newName))
            throw new SignalNotFoundException(newName,
                    "Attempt of set a existent name renaming a signal("+oldName+" to "+newName+")"); //@todo (Roman) comprobar si se deberia devolver true o false en vez de excepcion si existe la señal nueva
...
//    }
    ...
    public boolean setSignalHasEmphasisLevel(String signalName,boolean value){ // @todo (Document) No añadido a la documentacion
        if(exists(signalName)){
    ...
    public boolean getSignalHasEmphasisLevel(String signalName){ // @todo (Document) No añadido a la documentacion
        if(exists(signalName)){
