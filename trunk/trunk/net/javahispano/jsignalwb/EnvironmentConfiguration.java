  ...
  private static EnvironmentConfiguration environmentConfiguration;
    ...
    try {
      if (!archivo.exists()) {
        ...
        tmp.store(out, "Configuración de JSignalWorkBench");
        out.close();
      }
...
public String getPathJFFicheros(){
  ...
  /**
   * Almacena a disco el contenido actual del archivo de configuración.
   */
  public void almacenaADisco(){
    try {
      File archivo = new File(rutaDelArchivo);
      FileOutputStream out = new FileOutputStream(archivo);
      propiedades.store(out, "Configuración de JSignalWorkbench");
    }
    catch (IOException ex) {
      System.out.println("Cagamos en la configuración");
    }
